...
class FibHeap {
 public:
  ...
  static FibHeap* union_fibheap(FibHeap* H1, FibHeap* H2) {
    ...
    // 4
    if (H1->min == nullptr ||
        (H2->min != nullptr && H2->min->key < H1->min->key)) {
      // 5
      H->min = H2->min;
    }
    // 6
  ...
  void consolidate() {
    ...
    int max_degree = static_cast<int>(
        floor(log(static_cast<double>(n)) /
              log(static_cast<double>(1 + sqrt(static_cast<double>(5))) / 2)));

    // 1
    A = new FibNode*[max_degree + 2];  // plus two both for indexing to max
                                       // degree and so A[max_degree+1] == NIL
    // 2, 3
  ...
  FibNode* push(T k, int idx) {
    FibNode* x = new FibNode(k, idx);
    insert(x);
    return x;
  }
